<?php
    ...
    $idAsignProg = base64_decode($_GET["idAsign"]);

    $queryAsignProgs = "SELECT idAsignPrograma, asignatura, carrera, totalEstudiantes, primeraOp, segundaOp, totalAcreditados, 
    porcentajeAcreditados, estudiantesNoAcreditados, porcentajeNoAcreditados, desertores, porcentajeDesertores FROM asignprogramas WHERE idReporte = $idReporte AND idAsignPrograma = $idAsignProg";
    $resultAsign = mysqli_query($conn, $queryAsignProgs);
    $rowAsignProg = mysqli_fetch_array($resultAsign);
...
<script>
    ...
    if( totalEstudiantes.length < 1 || primeraOp.length < 1 || segundaOp.length < 1 || totalAcreditados.length < 1 || porcentajeAcreditados.length < 1 || 
        ...
        $.post('../controller/editar_asignatura.php?id=<?= $idReporte; ?>&idUs=<?= $idUsuario; ?>&idAsign=<?= $idAsignProg; ?>', { asignatura: asignatura, carrera: carrera,
            ...
            EstudiantesNoAcreditados: numNoAcreditados, porcentajeNoAcreditados: porcentajeNoAcreditados, desertores: numDesertores, porcentajeDesertores: porcentajeDesertores }, function(data){
                ...
                if(data!=null){
                    ...
                    location.href ="../views/ver_reporte.php?id=<?= base64_encode($idReporte);?>&idUs=<?= base64_encode($idUsuario);?>";
                } else{
