...
<body>
    ...
    <form class="contenedor"
        action="../controller/editar_planeacion.php?id=<?= $idPlaneacion; ?>&&idU=<?= $idUsuario; ?>" method="POST">
        ...
        <div class="card card-body col-md-8 mt-4" id="containerForm">
                    ...
                    <select class="form-select" name="selectMateria" id="selectMateria">
                        ...
                        <?php while ($rowMaterias = mysqli_fetch_array($resultMaterias)) { ?>
                            ...
                            <option <?= $rowPlaneacion['idMateria'] == $rowMaterias['idMateria'] ? "selected" : ""; ?>
                                value=<?= $rowMaterias['idMateria'] . "_" . $rowMaterias['idGrupo'];
                                ; ?>><?= utf8_decode($rowMaterias['nombreMateria']) . " - " . $rowMaterias['nombreGrupo']; ?>
                            </option>
            ...
            <div class="row col-md-12 mt-1">
                <div class="col-md-6 mt-2">
                    ...
                    <input type="text" readonly name="docente" id="docente" class="form-control"
                        value="<?= $nombreDocente; ?>">
                </div>
                <div class="col-md-6 mt-2">
                    <label for="selectPeriodo">Periodo</label>
                    <select class="form-select" name="selectPeriodo" id="selectPeriodo">
                        <option <?= $rowPlaneacion['periodo'] == $periodo1 . " " . $anio_actual ? "selected" : ""; ?>
                            value='<?= $periodo1 . " " . $anio_actual; ?>'> <?= $periodo1 . " " . $anio_actual; ?>
                        </option>
                        <option <?= $rowPlaneacion['periodo'] == $periodo2 . " " . $anio_actual ? "selected" : ""; ?>
                            value='<?= $periodo2 . " " . $anio_actual; ?>'> <?= $periodo2 . " " . $anio_actual; ?>
                        </option>
                        <option <?= $rowPlaneacion['periodo'] == $periodo3 . " " . $anio_actual ? "selected" : ""; ?>
                            value='<?= $periodo3 . " " . $anio_actual; ?>'> <?= $periodo3 . " " . $anio_actual; ?>
                        </option>
                    </select>
                </div>
            </div>
                ...
                <div class="col-md-6 mt-2">
                    ...
                    <select class="form-select" name="semana" id="semana">
                        ...
                        for ($i = 1; $i <= 16; $i++) {
                            ...
                            <option <?= $rowPlaneacion['noSemanas'] == $i ? "selected" : ""; ?> value="<?= $i ?>"><?= $i ?>
                            </option>
